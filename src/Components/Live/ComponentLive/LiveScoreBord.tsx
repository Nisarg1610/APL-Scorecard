...
const matchData = {
  teamA: {
    ...
    players: [
      { name: 'Virat Kohli', runs: 85, fours: 6, sixes: 4, balls: 60, out: false },
      { name: 'Rohit Sharma', runs: 75, fours: 7, sixes: 3, balls: 55, out: false },
  ...
  teamB: {
    ...
    players: [
      { name: 'David Warner', runs: 65, fours: 5, sixes: 3, balls: 45, out: false },
      { name: 'Steve Smith', runs: 70, fours: 6, sixes: 2, balls: 50, out: false },
      { name: 'Glenn Maxwell', runs: 25, fours: 3, sixes: 1, balls: 30, out: true },
    ],
  },
};
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-md">
      ...
      <div className="flex justify-between items-center gap-2 bg-gray-200">
        <button
          className={`w-full py-2 ${selectedTeam === 'teamA' ? 'border-b-2 border-black bg-gray-300' : ''}`}
          onClick={() => handleTeamChange('teamA')}
        >
          {matchData.teamA.name}
        </button>
        <button
          className={`w-full py-2 ${selectedTeam === 'teamB' ? 'border-b-2 border-black bg-gray-300' : ''}`}
            ...
            <tbody>
              {selectedTeamData.players.map((player, index) => (
                <tr key={index} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">{calculateStrikeRate(player.runs, player.balls)}</td>
                  <td className="px-4 py-2">
                    {player.out ? (
                      <span className="text-red-600 text-sm">(Out)</span>
                    ) : (
                      <span className="text-green-600 text-sm">(Playing)</span>
                    )}
                  </td>
                </tr>
              ))}
